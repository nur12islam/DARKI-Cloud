...
function pkceChallenge(verifier: string): string { return base64Url(createHash("sha256").update(verifier, "utf8").digest()); }

export type TelegramOidcConfig = { clientId: string; clientSecret: string; redirectUri: string; appRedirectUri: string };

export class TelegramOidcService {
  constructor(private readonly config: TelegramOidcConfig, private readonly auth = new AuthService()) {}
  ...
  async callback(code: string, state: string): Promise<string> {
    const result = await db.query<{ id: string; codeVerifier: string }>(`SELECT id, code_verifier AS "codeVerifier" FROM telegram_login_attempts WHERE state_hash = $1 AND used_at IS NULL AND expires_at > now()`, [hash(state)]);
    ...
    const tokenResponse = await fetch(TELEGRAM_TOKEN_URL, {
      ...
      body: new URLSearchParams({ grant_type: "authorization_code", code, redirect_uri: this.config.redirectUri, client_id: this.config.clientId, code_verifier: attempt.codeVerifier }),
    });
