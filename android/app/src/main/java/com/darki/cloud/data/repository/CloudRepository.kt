...
class CloudRepository(private val api: DarkiCloudApi, private val dao: CloudDao) {
    ...
    suspend fun removeTransfer(id: String) = dao.deleteTransfer(id)
    ...
    suspend fun uploadFile(token: String, folderId: String, name: String, mimeType: String?, sizeBytes: Long, deviceId: String, operationId: String, input: InputStream, onProgress: ((Long) -> Unit)? = null): FileEntity = api.uploadFile(token, folderId, name, mimeType, sizeBytes, deviceId, operationId, input, onProgress).getJSONObject("file").also { dao.upsertFiles(listOf(it.toFileEntity(it.getString("userId")))) }.toFileEntity()
    suspend fun downloadFile(token: String, fileId: String, output: OutputStream, onProgress: ((Long) -> Unit)? = null) = api.downloadFile(token, fileId, output, onProgress)
    ...
    suspend fun deleteFile(token: String, fileId: String, deviceId: String) { api.deleteFile(token, fileId, deviceId); dao.findFileById(fileId)?.let { dao.markFileDeleted(fileId, System.currentTimeMillis()) } }
    ...
    private fun JSONObject.toDeviceEntity(): DeviceEntity = DeviceEntity(id = getString("id"), userId = getString("userId"), name = optString("name").ifBlank { getString("deviceName") }, platform = getString("platform"), lastSeenAt = optTimestamp("lastSeenAt"), syncCursor = optLong("syncCursor", 0L))
