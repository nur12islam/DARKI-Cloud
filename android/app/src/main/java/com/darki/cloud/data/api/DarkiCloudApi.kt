...
class DarkiCloudApi(
    ...
    suspend fun moveFolder(token: String, folderId: String, parentId: String, deviceId: String): JSONObject = patchJson("/api/v1/folders/$folderId", token, JSONObject().apply { put("parentId", parentId); put("deviceId", deviceId) })
    suspend fun renameFile(token: String, fileId: String, name: String, deviceId: String): JSONObject = patchJson("/api/v1/files/$fileId", token, JSONObject().apply { put("name", name); put("deviceId", deviceId) })
