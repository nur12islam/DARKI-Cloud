...
private fun DarkiCloudApp(viewModel: DarkiCloudViewModel, authCode: String?, onLogin: () -> Unit) {
    ...
    MaterialTheme {
        Surface(modifier = Modifier.fillMaxSize(), color = Color(0xFF050505)) {
            ...
            var downloadTarget by remember { mutableStateOf<FileEntity?>(null) }
            val filePicker = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
                if (uri != null) viewModel.uploadFile(uri, context.contentResolver)
            }
            val saveFilePicker = rememberLauncherForActivityResult(ActivityResultContracts.CreateDocument("application/octet-stream")) { uri ->
                val file = downloadTarget
                downloadTarget = null
                if (uri != null && file != null) viewModel.downloadFile(file, context.contentResolver, context.contentResolver.openOutputStream(uri) ?: return@rememberLauncherForActivityResult)
            }

            if (authenticated && stack.isNotEmpty()) BackHandler { viewModel.navigateBack() }
            if (showCreateFolder) {
                CreateFolderDialog(onDismiss = { showCreateFolder = false }, onCreate = { name -> showCreateFolder = false; viewModel.createFolder(name) })
            }
            ...
            Box(Modifier.fillMaxSize()) {
                Column(Modifier.fillMaxSize()) {
                    ...
                    if (authenticated) DriveContent(folders, files, error, viewModel::openFolder, { file -> downloadTarget = file; saveFilePicker.launch(file.name) })
                ...
                if (authenticated) {
                    FloatingActionButton(onClick = { filePicker.launch(arrayOf("*/*")) }, modifier = Modifier.align(Alignment.BottomEnd).padding(24.dp), containerColor = Color(0xFF171717), contentColor = Color(0xFFB7F7FF)) {
                        if (uploading || downloading) CircularProgressIndicator(Modifier.size(24.dp), strokeWidth = 2.dp) else Icon(Icons.Default.UploadFile, contentDescription = "Upload file")
                    }
                    if (!uploading && !downloading) FloatingActionButton(onClick = { showCreateFolder = true }, modifier = Modifier.align(Alignment.BottomEnd).padding(end = 24.dp, bottom = 96.dp), containerColor = Color(0xFF171717), contentColor = Color(0xFFB7F7FF)) { Icon(Icons.Default.CreateNewFolder, contentDescription = "New folder") }
                }
            }
        }
    }
...
private fun DriveTopBar(refreshing: Boolean, authenticated: Boolean, canGoBack: Boolean, onBack: () -> Unit, onRefresh: () -> Unit, onLogout: () -> Unit, onLogin: () -> Unit) {
    Column(Modifier.padding(horizontal = 20.dp, vertical = 18.dp)) {
        Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            ...
            Icon(Icons.Default.Cloud, null, tint = Color(0xFFB7F7FF), modifier = Modifier.size(30.dp)); Spacer(Modifier.width(12.dp))
...
private fun LoginContent(onLogin: () -> Unit) {
    Column(Modifier.fillMaxSize().padding(horizontal = 28.dp), horizontalAlignment = Alignment.CenterHorizontally) {
        Spacer(Modifier.height(120.dp)); Icon(Icons.Default.Cloud, null, tint = Color(0xFFB7F7FF), modifier = Modifier.size(72.dp)); Spacer(Modifier.height(22.dp))
        Text("Your private cloud", style = MaterialTheme.typography.headlineMedium, fontWeight = FontWeight.Bold); Text("Sign in with Telegram to access your files.", color = Color(0xFF858585), modifier = Modifier.padding(top = 8.dp)); Spacer(Modifier.height(28.dp))
...
private fun DriveContent(folders: List<FolderEntity>, files: List<FileEntity>, error: String?, onFolderClick: (FolderEntity) -> Unit, onFileDownload: (FileEntity) -> Unit) {
    Column(Modifier.fillMaxSize().padding(horizontal = 20.dp)) {
        ...
        LazyColumn(verticalArrangement = Arrangement.spacedBy(10.dp), contentPadding = PaddingValues(bottom = 100.dp)) {
            ...
            items(files, key = { it.id }) { file -> DriveItemRow(file.name, formatSize(file.sizeBytes), false) { onFileDownload(file) } }
        }
    }
}
...
private fun DriveItemRow(name: String, subtitle: String, isFolder: Boolean, onClick: () -> Unit) {
    Row(Modifier.fillMaxWidth().clip(RoundedCornerShape(18.dp)).background(Color(0xFF101010)).padding(horizontal = 16.dp, vertical = 15.dp), verticalAlignment = Alignment.CenterVertically) {
        ...
        Spacer(Modifier.width(4.dp)); Column(Modifier.weight(1f)) { Text(name, fontWeight = FontWeight.Medium); Text(subtitle, style = MaterialTheme.typography.labelSmall, color = Color(0xFF777777), modifier = Modifier.padding(top = 3.dp)) }
        if (!isFolder) IconButton(onClick = onClick) { Icon(Icons.Default.Download, "Download", tint = Color(0xFF888888)) }
    }
}
...
private fun formatSize(bytes: Long?): String {
    if (bytes == null) return "File"; if (bytes < 1024) return "$bytes B"; if (bytes < 1024 * 1024) return "%.1f KB".format(bytes / 1024.0); if (bytes < 1024 * 1024 * 1024) return "%.1f MB".format(bytes / (1024.0 * 1024.0)); return "%.1f GB".format(bytes / (1024.0 * 1024.0 * 1024.0))
}
